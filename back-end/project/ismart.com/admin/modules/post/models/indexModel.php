...
function pagging_post($sql, $where = "", $num_per_page = 2)
{
    ...
    $list_pages = get_pages_post($start, $num_per_page, "{$where}");
    $result = array(
        'num_page' => $num_page,
        'page' => $page,
...
function get_pages_post($start = 1, $num_per_page = 10, $where = "")
{
    if (!empty($where)) {
        $where = "WHERE {$where} ";
    }
...
{
    $result = db_fetch_row("SELECT * FROM `tbl_posts` WHERE `id` = '{$id}'");
    if (!empty($result)) {
        return $result;
    } else {
        return FALSE;
...
function update_post_info($data, $id){
