...
function indexAction()
{
    if (isset($_GET['search'])) {
        if ($_GET['search_by'] == 'all') {
            ...
            $result = pagging_slider("SELECT * FROM `tbl_slider` WHERE `title` LIKE '%$search_value%' OR `created_name` LIKE '%$search_value%'", "`title` LIKE '%$search_value%' OR `created_name` LIKE '%$search_value%'", 5);
            // show_array($result['list_pages']);
            $data = array(
                'num_page' => $result['num_page'],
                'page' => $result['page'],
                'slider_list' => $result['list_pages'],
                'search_by' => $_GET['search_by'],
                'search_value' => $search_value
            );
        }
        if ($_GET['search_by'] == 'slider_title') {
            $search_value = $_GET['search_value'];
            $result = pagging_slider("SELECT * FROM `tbl_slider` WHERE `title` LIKE '%$search_value%'", "`title` LIKE '%$search_value%'", 5);
            // show_array($result['list_pages']);
            $data = array(
                'num_page' => $result['num_page'],
                'page' => $result['page'],
        ...
        if ($_GET['search_by'] == 'created_name') {
            $search_value = $_GET['search_value'];
            $result = pagging_slider("SELECT * FROM `tbl_slider` WHERE `created_name` LIKE '%$search_value%'", "`created_name` LIKE '%$search_value%'", 5);
            // show_array($result['list_pages']);
            $data = array(
                'num_page' => $result['num_page'],
                'page' => $result['page'],
        ...
        $result = pagging_slider("SELECT * FROM `tbl_slider`", "", 5);
        $data = array(
            'num_page' => $result['num_page'],
            'page' => $result['page'],
            'slider_list' => $result['list_pages']
...
function add_sliderAction()
{
    ...
    if (isset($_POST['btn_submit'])) {
        ...
        if (empty($_POST['title'])) {
            $error['title'] = "Khong duoc de trong ten slider";
        } else {
            $title = $_POST['title'];
        }
        if (empty($_POST['slug'])) {
            $error['link'] = "Khong duoc de trong link ";
        } else {
            $link = $_POST['slug'];
        }
        if (empty($_POST['num_order'])) {
            $error['num_order'] = "Khong duoc de trống thứ tự";
        } else {
            $num_order = $_POST['num_order'];
        }
        if (empty($_POST['status'])) {
            $error['status'] = "Khong duoc de trong trang thai slider ";
        } else {
            $status = $_POST['status'];
        }
        if (empty($_POST['desc'])) {
            $error['desc'] = "Khong duoc de trong mo ta slider";
        } else {
...
function editAction()
{
    ...
    if (isset($_POST['btn_submit'])) {
        ...
        if (empty($error)) {
            ...
            update_slider_info($data, $id);
        }
    }
    $slider_info = get_slider_info($id);
